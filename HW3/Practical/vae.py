...
Original file is located at
    https://colab.research.google.com/drive/1kLiWAZyCzlHDrg0Y4cZuiqjozIHtoSsO
...
class DiagonalGaussianDistribution(object):
  ...
  def sample(self):
    ...
    sample = torch.randn_like(self.mean) * self.std + self.mean
    return sample
  ...
  def kl(self):
    ...
    kl_div = 0.5 * torch.sum(torch.exp(self.logvar) + self.mean**2 - 1. - self.logvar, dim=1)
    return kl_div
  ...
  def nll(self, sample, dims=[1, 2, 3]):
    ...
    negative_ll = 0.5 * torch.sum(np.log(2. * np.pi) + self.logvar + (sample - self.mean)**2 / self.var, dim=dims)
    return negative_ll
...
class VAE(nn.Module):
  ...
  def encode(self, x):
    ...
    return DiagonalGaussianDistribution(self.mean(self.encoder(x)), self.logvar(self.encoder(x)))
  ...
  def decode(self, z):
    ...
    return DiagonalGaussianDistribution(self.decoder(z))
  ...
  def sample(self, batch_size):
    ...
    return self.decode(torch.randn(batch_size, self.z_dim).to(self.device)).mode()
  ...
  def log_likelihood(self, x, K=100):
    ...
    for i in range(K):
      ...
      z = posterior.sample()
      #log_likelihood[:, i] = None     # WRITE CODE HERE (compute the log-likelihood of the sample)
      ...
      log_likelihood[:, i] = - recon.nll(x) + posterior.nll(z, dims = [1]) - prior.nll(z, dims = [1])
    #ll = None     # WRITE CODE HERE (compute the final log-likelihood using the log-sum-exp trick)
    ll = torch.logsumexp(log_likelihood, dim=1) - torch.log(torch.tensor(K).to(self.device))
    return ll
